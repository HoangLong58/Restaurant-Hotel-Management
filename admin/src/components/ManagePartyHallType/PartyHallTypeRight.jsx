...
const PartyHallTypeRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng thiết bị
    const [quantityPartyHallType, setQuantityPartyHallType] = useState();
    useEffect(() => {
        const getQuantityPartyHallType = async () => {
            try {
                const quantityPartyHallTypeRes = await PartyHallTypeService.getQuantityPartyHallType();
                setQuantityPartyHallType(quantityPartyHallTypeRes.data.data.quantityPartyHallType);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
        getQuantityPartyHallType();
    }, [reRenderData]);
    console.log("Số lượng loại SẢNH TIỆC: ", quantityPartyHallType);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG LOẠI SẢNH TIỆC</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityPartyHallType}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createPartyHallType" })}
                >
                    <Add />
                    <h3>Thêm Loại Sảnh tiệc</h3>
                </Item>
            </SalesAnalytics>
...
export default PartyHallTypeRight;