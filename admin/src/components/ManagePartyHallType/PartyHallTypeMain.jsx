...
const PartyHallTypeMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findPartyHallType = async () => {
                try {
                    const searchRes = await PartyHallTypeService.findPartyHallTypeByIdOrName(search);
                    ...
                    setPartyHallTypeList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - PartyHallTypeMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getPartyHallTypes = async () => {
            try {
                const partyHallTypeRes = await PartyHallTypeService.getAllPartyHallTypes();
                setPartyHallTypeList(partyHallTypeRes.data.data);
            } catch (err) {
                console.log("Lỗi lấy party hall type: ", err);
            }
        }
        handleLoading();
        getPartyHallTypes();
    }, [reRenderData]);
    console.log("partyHallTypeList: ", partyHallTypeList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [partyHallTypeModal, setPartyHallTypeModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setPartyHallTypeModal(modal.partyHallType);
    }
    ...
    const partyHallTypePerPage = 12;
    const pageVisited = pageNumber * partyHallTypePerPage;

    const partyHallTypeListFiltered = partyHallTypeList
        .slice(pageVisited, pageVisited + partyHallTypePerPage)
        .map((partyHallType, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{pageNumber * partyHallTypePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{partyHallType.party_hall_type_id}</Td>
                    <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{partyHallType.party_hall_type_name}</Td>
                    <Td
                        onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}
                        style={{ backgroundColor: partyHallType.party_hall_type_state === 0 ? "var(--color-info)" : partyHallType.party_hall_type_state === 1 ? "var(--color-danger)" : null }}>
                        {partyHallType.party_hall_type_state === 0 ? "Đang hoạt động" : partyHallType.party_hall_type_state === 1 ? "Ngưng hoạt động" : null}
                    </Td>
                    <Td className="warning">
                        <ButtonFix
                            onClick={() => openModal({ type: "updatePartyHallType", partyHallType: partyHallType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "disablePartyHallType", partyHallType: partyHallType })}
                        >
                            <PersonOffOutlined />
                        </ButtonDelete>
                    </Td>
                    <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "ablePartyHallType", partyHallType: partyHallType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deletePartyHallType", partyHallType: partyHallType })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(partyHallTypeList.length / partyHallTypePerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Loại Sảnh tiệc - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Loại Sảnh tiệc - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                            <Th>Mã Loại Sảnh tiệc</Th>
                            <Th>Tên Loại Sảnh tiệc</Th>
                            <Th>Trạng thái</Th>
                            <Th>Chỉnh sửa</Th>
                            <Th>Vô hiệu hóa</Th>
                            <Th>Mở khóa</Th>
                    ...
                    <Tbody>
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                partyHallTypeList.length > 0
                                    ?
                                    partyHallTypeListFiltered
                                    :
                                    (partyHallTypeList.map((partyHallType, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{partyHallType.party_hall_type_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}>{partyHallType.party_hall_type_name}</Td>
                                                <Td
                                                    onClick={() => openModal({ type: "detailPartyHallType", partyHallType: partyHallType })}
            ...
            <Modal
                ...
                partyHallType={partyHallTypeModal}  //Dữ liệu bên trong modal
...
export default PartyHallTypeMain;