...
const DeviceMain = ({ reRenderData, setReRenderData }) => {
    // Search
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            const findDevice = async () => {
                try {
                    const searchRes = await DeviceService.findDeviceByIdOrName(search);
    ...
    useEffect(() => {
        const getDevices = async () => {
            try {
                ...
                setDeviceList(deviceRes.data.data);
            } catch (err) {
    ...
    const pageVisited = pageNumber * devicePerPage;

    const deviceFiltered = deviceList
        .slice(pageVisited, pageVisited + devicePerPage)
        .map((device, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{pageNumber * devicePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_id}</Td>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_type_name}</Td>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_name}</Td>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <ImgDanhMuc src={device.device_image} />
                    </Td>
                    <Td onClick={() => openModal({ type: "detailDevice", device: device })} style={{ backgroundColor: device.device_state === 0 ? "var(--color-info)" : device.device_state === 1 ? "var(--color-success)" : null }}>
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteDevice", device: device })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td> */}
    ...
    const pageCount = Math.ceil(deviceList.length / devicePerPage);
    const changePage = ({ selected }) => {
        setPageNumber(selected);
    }
    return (
        <Container>
            <H2>Quản lý Thiết bị - Khách sạn</H2>
            ...
            <RecentOrders>
                <H2>Thiết bị - Khách sạn hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                deviceList.length > 0
                                    ...
                                    deviceFiltered
                                    :
                                    (deviceList.slice(0, 12).map((device, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{key + 1}</Td>
                                                <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_type_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailDevice", device: device })}>{device.device_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailDevice", device: device })} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Modal
                ...
                device={deviceModal}  //Dữ liệu bên trong modal
