...
const FoodMain = ({ reRenderData, setReRenderData }) => {
    // Search
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            const findFood = async () => {
                try {
                    const searchRes = await FoodService.findFoodByIdOrName(search);
                    ...
                    setFoodList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - FoodMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getFoods = async () => {
            try {
                const foodRes = await FoodService.getAllFoods();
    ...
    const foodFiltered = foodList
        .slice(pageVisited, pageVisited + foodPerPage)
        .map((food, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailFood", food: food })}>{pageNumber * foodPerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_id}</Td>
                    <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_type_name}</Td>
                    <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_name}</Td>
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteFood", food: food })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td> */}
    ...
    const pageCount = Math.ceil(foodList.length / foodPerPage);
    const changePage = ({ selected }) => {
        setPageNumber(selected);
    }
    return (
        <Container>
            <H2>Quản lý Món ăn - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Món ăn - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                foodList.length > 0
                                    ...
                                    foodFiltered
                                    :
                                    (foodList.slice(0, 12).map((food, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailFood", food: food })}>{key + 1}</Td>
                                                <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_type_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailFood", food: food })}>{food.food_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailFood", food: food })} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Modal
                ...
                food={foodModal}  //Dữ liệu bên trong modal
