...
const TableBookingRight = ({ reRenderData, setReRenderData }) => {
    ...
    // Số lượng Đặt bàn
    const [tableBookingOrderQuantity, setTableBookingOrderQuantity] = useState();
    useEffect(() => {
        const getTableBookingOrderQuantity = async () => {
            try {
                const roomQuantityRes = await TableBookingOrderService.getQuantityTableBooking();
                setTableBookingOrderQuantity(roomQuantityRes.data.data.quantityTableBooking);
            } catch (err) {
                console.log("Lỗi: ", err.response);
            }
        }
        getTableBookingOrderQuantity();
    }, [reRenderData]);
    console.log("Số lượng Đặt bàn: ", tableBookingOrderQuantity);
