...
const TableBookingMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            const findTableBooking = async () => {
                try {
                    const searchRes = await TableBookingOrderService.findTableBookingByIdOrCustomerEmailOrCustomerPhoneOrCustomerName(search);
                    ...
                    setTableBookingOrderList(searchRes.data.data);
                    // Toast
    ...
    // Lấy table booking order list
    const [tableBookingOrderList, setTableBookingOrderList] = useState([]);
    useEffect(() => {
        const getTableBookingOrderList = async () => {
            try {
                const getTableBookingOrderRes = await TableBookingOrderService.getTableBookingAndDetails();
                setTableBookingOrderList(getTableBookingOrderRes.data.data);
            } catch (err) {
                console.log("Lỗi lấy table booking order: ", err.response);
            }
        }
        handleLoading();
        getTableBookingOrderList();
    }, [reRenderData]);
    console.log("tableBookingOrderList: ", tableBookingOrderList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [tableBookingOrderModal, setTableBookingOrderModal] = useState(null);
    const [tableBookingOrderAddFoodModal, setTableBookingOrderAddFoodModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setTableBookingOrderModal(modal.tableBookingOrder);
        setTableBookingOrderAddFoodModal(modal.tableBookingOrder);
    }
                ...
                return (
                    <>
                        <Td className="primary">
                            <ButtonInfo
                                onClick={() => openModal({ type: "checkin", tableBookingOrder: data })}
                            >
                        ...
                        <Td className="primary">
                            <ButtonInfo
                                onClick={() => openModal({ type: "addFoodToTable", tableBookingOrder: data })}
                            >
                        ...
                        <Td className="primary">
                            <ButtonDelete
                                onClick={() => openModal({ type: "checkout", tableBookingOrder: data })}
                            >
    ...
    const tableBookingOrderPerPage = 12;
    const pageVisited = pageNumber * tableBookingOrderPerPage;

    const tableBookingOrderListFiltered = tableBookingOrderList
        .slice(pageVisited, pageVisited + tableBookingOrderPerPage)
        .map((tableBookingOrder, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{pageNumber * tableBookingOrderPerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.table_booking_order_id}</Td>
                    <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.table_booking_name + " - " + tableBookingOrder.table_type_name + ", " + tableBookingOrder.floor_name}</Td>
                    <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.customer_first_name + " " + tableBookingOrder.customer_last_name}</Td>
                    ...
                    <Td
                        ...
                        {tableBookingOrder.table_booking_order_state === 0 ? "Đã đặt" : tableBookingOrder.table_booking_order_state === 1 ? "Đã xác nhận" : tableBookingOrder.table_booking_order_state === 2 ? "Hoàn thành" : null}
                    </Td>

                    {authorizationAdminTableData(admin, tableBookingOrder)}
                    {/* <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "checkin", tableBookingOrder: tableBookingOrder })}
                        >
                            <AssignmentOutlined style={{ color: "var(--color-info)" }} />
                        </ButtonInfo>
                    </Td>
                    <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "addFoodToTable", tableBookingOrder: tableBookingOrder })}
                        >
                            <PostAddOutlined style={{ color: "var(--color-info)" }} />
                        </ButtonInfo>
                    </Td>
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "checkout", tableBookingOrder: tableBookingOrder })}
                        >
                            <AssignmentTurnedInOutlined />
                        </ButtonDelete>
                    </Td> */}
    ...
    const pageCount = Math.ceil(tableBookingOrderList.length / tableBookingOrderPerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Đặt Bàn - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Danh sách Đặt bàn hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                    ...
                    <Tbody>
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                tableBookingOrderList.length > 0
                                    ?
                                    tableBookingOrderListFiltered
                                    :
                                    (tableBookingOrderList.slice(0, 12).map((tableBookingOrder, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.table_booking_order_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.table_booking_name + " - " + tableBookingOrder.table_type_name + ", " + tableBookingOrder.floor_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.customer_first_name + " " + tableBookingOrder.customer_last_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableBookingOrder", tableBookingOrder: tableBookingOrder })}>{tableBookingOrder.table_booking_order_checkin_date}</Td>
            ...
            <Modal
                ...
                tableBookingOrder={tableBookingOrderModal}  //Dữ liệu bên trong modal
                tableBookingOrderAddFood={tableBookingOrderAddFoodModal}  //Dữ liệu bên trong modal
