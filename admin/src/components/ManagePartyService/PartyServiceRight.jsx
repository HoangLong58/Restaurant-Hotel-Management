...
const PartyServiceRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng danh mục
    const [quantityPartyService, setQuantityPartyService] = useState();
    useEffect(() => {
        const getQuantityPartyService = async () => {
            try {
                const quantityPartyServiceRes = await PartyServiceService.getQuantityPartyService();
                setQuantityPartyService(quantityPartyServiceRes.data.data.quantityPartyService);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
        getQuantityPartyService();
    }, [reRenderData]);
    console.log("Số lượng danh mục: ", quantityPartyService);
    ...
    return (
        <Container>
            <RightTop />
            <SalesAnalytics>
                <H2>Party Services Analytics</H2>
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG DỊCH VỤ TIỆC</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityPartyService}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createPartyService" })}
                >
                    <Add />
                    <h3>Thêm Dịch vụ Tiệc</h3>
                </Item>
            </SalesAnalytics>
...
export default PartyServiceRight;