...
const PartyServiceMain = ({ reRenderData, setReRenderData }) => {
    // Search
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findPartyService = async () => {
                try {
                    const searchRes = await PartyServiceService.findPartyServiceByIdOrName(search);
                    ...
                    setPartyServiceList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - PartyServiceMain & DanhMucRight.jsx
    }

    // Lấy danh mục
    const [partyServiceList, setPartyServiceList] = useState([]);
    useEffect(() => {
        const getPartyServices = async () => {
            try {
                const partyServiceRes = await PartyServiceService.getAllPartyServices();
                setPartyServiceList(partyServiceRes.data.data);
            } catch (err) {
                console.log("Lỗi lấy party service: ", err);
            }
        }
        handleLoading();
        getPartyServices();
    }, [reRenderData]);
    console.log("partyServiceList: ", partyServiceList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [partyServiceModal, setPartyServiceModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setPartyServiceModal(modal.partyService);
    }
    ...
    const partyServicePerPage = 12;
    const pageVisited = pageNumber * partyServicePerPage;

    const partyServiceFiltered = partyServiceList
        .slice(pageVisited, pageVisited + partyServicePerPage)
        .map((partyService, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{pageNumber * partyServicePerPage + (key + 1)}</Td>
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deletePartyService", partyService: partyService })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(partyServiceList.length / partyServicePerPage);
    const changePage = ({ selected }) => {
        setPageNumber(selected);
    }
    return (
        <Container>
            <H2>Quản lý Dịch vụ Tiệc - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Dịch vụ Tiệc - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                            <Th>Mã Dịch vụ Tiệc</Th>
                    ...
                    <Tbody>
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                partyServiceList.length > 0
                                    ?
                                    partyServiceFiltered
                                    :
                                    (partyServiceList.slice(0, 12).map((partyService, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{partyService.party_service_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{partyService.party_service_type_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{partyService.party_service_name}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyService", partyService: partyService })}>{partyService.party_service_price}</Td>
            ...
            <Modal
                ...
                partyService={partyServiceModal}  //Dữ liệu bên trong modal
...
export default PartyServiceMain;