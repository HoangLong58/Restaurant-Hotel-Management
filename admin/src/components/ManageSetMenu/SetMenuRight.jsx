...
const SetMenuRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng danh mục
    const [quantitySetMenu, setQuantitySetMenu] = useState();
    useEffect(() => {
        const getQuantitySetMenu = async () => {
            try {
                const quantitySetMenuRes = await SetMenuService.getQuantitySetMenu();
                setQuantitySetMenu(quantitySetMenuRes.data.data.quantitySetMenu);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
    ...
    console.log("Số lượng danh mục: ", quantitySetMenu);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG SET MENU</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantitySetMenu}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createSetMenu" })}
                >
                    <Add />
                    <h3>Thêm Set Menu</h3>
                </Item>
