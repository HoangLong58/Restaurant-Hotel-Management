...
const PartyBookingRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng Đặt phòng
    const [partyBookingOrderQuantity, setPartyBookingOrderQuantity] = useState();
    useEffect(() => {
        const getPartyBookingOrderQuantity = async () => {
            try {
                const roomQuantityRes = await PartyBookingOrderService.getQuantityPartyBooking();
                setPartyBookingOrderQuantity(roomQuantityRes.data.data.quantityPartyBooking);
            } catch (err) {
                console.log("Lỗi: ", err.response);
            }
        }
        getPartyBookingOrderQuantity();
    }, [reRenderData]);
    console.log("Số lượng Đặt phòng: ", partyBookingOrderQuantity);
    ...
    return (
        <Container>
            <RightTop />
            <SalesAnalytics>
                <H2>Party Booking Orders Analytics</H2>
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG ĐẶT TIỆC</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{partyBookingOrderQuantity}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "statisticPartyBooking" })}
                >
                    <Add />
                    <h3>Tìm kiếm &amp; Thống kê Đặt tiệc Từng thành phố dựa vào Doanh thu cả năm</h3>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "statisticPartyBookingTotal" })}
                >
                    <Add />
...
export default PartyBookingRight;