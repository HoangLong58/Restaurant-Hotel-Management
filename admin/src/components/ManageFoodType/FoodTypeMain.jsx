...
const FoodTypeMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            const findFoodType = async () => {
                try {
                    const searchRes = await FoodTypeService.findFoodTypeByIdOrName(search);
                    ...
                    setFoodTypeList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - FoodTypeMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getFoodTypes = async () => {
            try {
                ...
                console.log("Lỗi lấy food type: ", err);
            }
        }
        handleLoading();
        getFoodTypes();
    ...
    const [foodTypeModal, setFoodTypeModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setFoodTypeModal(modal.foodType);
    }
    ...
    const foodTypePerPage = 12;
    const pageVisited = pageNumber * foodTypePerPage;

    const foodTypeListFiltered = foodTypeList
        .slice(pageVisited, pageVisited + foodTypePerPage)
        .map((foodType, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{pageNumber * foodTypePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{foodType.food_type_id}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{foodType.food_type_name}</Td>
                    <Td
                        style={{ backgroundColor: foodType.food_type_state === 0 ? "var(--color-info)" : foodType.food_type_state === 1 ? "var(--color-danger)" : null }}
                        onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}
                    >
                        {foodType.food_type_state === 0 ? "Đang hoạt động" : foodType.food_type_state === 1 ? "Đã đóng" : null}
                    </Td>

                    <Td className="warning">
                        <ButtonFix
                            onClick={() => openModal({ type: "updateFoodType", foodType: foodType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteFoodType", foodType: foodType })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(foodTypeList.length / foodTypePerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Loại món ăn - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Loại món ăn - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                foodTypeList.length > 0
                                    ?
                                    foodTypeListFiltered
                                    :
                                    (foodTypeList.map((foodType, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{key + 1}</Td>
                                                <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{foodType.food_type_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}>{foodType.food_type_name}</Td>
                                                <Td
                                                    style={{ backgroundColor: foodType.food_type_state === 0 ? "var(--color-info)" : foodType.food_type_state === 1 ? "var(--color-danger)" : null }}
                                                    onClick={() => openModal({ type: "detailFoodType", foodType: foodType })}
                                                >
                                                    {foodType.food_type_state === 0 ? "Đang hoạt động" : foodType.food_type_state === 1 ? "Ngưng hoạt động" : null}
                                                </Td>
                                                <Td className="warning">
                                                    <ButtonFix
                                                        onClick={() => openModal({ type: "updateFoodType", foodType: foodType })}
            ...
            <Modal
                ...
                foodType={foodTypeModal}  //Dữ liệu bên trong modal
