...
const FoodVoteRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng bình luận
    const [quantityFoodVote, setQuantityFoodVote] = useState();
    useEffect(() => {
        const getQuantityFoodVote = async () => {
            try {
                const quantityFoodVoteRes = await FoodVoteService.getQuantityFoodVote();
                setQuantityFoodVote(quantityFoodVoteRes.data.data.quantityFoodVote);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
    ...
    console.log("Số lượng loại bình luận: ", quantityFoodVote);
    ...
    return (
        <Container>
            <RightTop />
            <SalesAnalytics>
                <H2>Phân tích Bình luận - Đánh giá</H2>
                <Item className="online">
                    ...
                    <ItemRight>
                        ...
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityFoodVote}</h3>
                    </ItemRight>
                </Item>
