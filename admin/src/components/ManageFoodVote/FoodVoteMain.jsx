...
const FoodVoteMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findfoodVote = async () => {
                try {
                    const searchRes = await FoodVoteService.findFoodVoteByIdOrName(search);
                    ...
                    setFoodVoteList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - FoodVoteMain & DanhMucRight.jsx
    }

    // Lấy danh mục
    const [foodVoteList, setFoodVoteList] = useState([]);
    useEffect(() => {
        const getFoodVotes = async () => {
            try {
                const foodVotesRes = await FoodVoteService.getAllFoodVotes();
                setFoodVoteList(foodVotesRes.data.data);
            } catch (err) {
    ...
    console.log("foodVoteList: ", foodVoteList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [foodVoteModal, setFoodVoteModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setFoodVoteModal(modal.foodVote);
    }
    ...
    const foodVotePerPage = 12;
    const pageVisited = pageNumber * foodVotePerPage;

    const foodVoteListFiltered = foodVoteList
        .slice(pageVisited, pageVisited + foodVotePerPage)
        .map((foodVote, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{pageNumber * foodVotePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_vote_id}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_name}</Td>
                    <Td
                        onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}
                        style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <ImgDanhMuc src={foodVote.food_image} />
                    </Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_vote_number}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.customer_first_name + " " + foodVote.customer_last_name}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_vote_comment}</Td>
                    <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })} style={{ backgroundColor: foodVote.food_vote_reply ? "white" : "#f5f5f5" }}>{foodVote.food_vote_reply ? foodVote.food_vote_reply : "Chưa trả lời bình luận này!"}</Td>
                    <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "addEmployeeComment", foodVote: foodVote })}
                        >
                            <AddCommentOutlined style={{ color: "var(--color-info)" }} />
                        </ButtonInfo>
                    </Td>
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteFoodVote", foodVote: foodVote })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(foodVoteList.length / foodVotePerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Bình luận Món ăn Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Bình luận Món ăn Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                    ...
                    <Tbody>
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                foodVoteList.length > 0
                                    ?
                                    foodVoteListFiltered
                                    :
                                    (foodVoteList.map((foodVote, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{key + 1}</Td>
                                                <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_vote_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}>{foodVote.food_name}</Td>
                                                <Td
                                                    onClick={() => openModal({ type: "detailFoodVote", foodVote: foodVote })}
            ...
            <Modal
                ...
                foodVote={foodVoteModal}  //Dữ liệu bên trong modal
