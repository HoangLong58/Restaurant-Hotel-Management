...
const TableTypeRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng thiết bị
    const [quantityTableType, setQuantityTableType] = useState();
    useEffect(() => {
        const getQuantityTableType = async () => {
            try {
                const quantityTableTypeRes = await TableTypeService.getQuantityTableType();
                setQuantityTableType(quantityTableTypeRes.data.data.quantityTableType);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
        getQuantityTableType();
    }, [reRenderData]);
    console.log("Số lượng loại bàn: ", quantityTableType);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG LOẠI BÀN</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityTableType}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createTableType" })}
                >
                    <Add />
                    <h3>Thêm Loại bàn</h3>
                </Item>
