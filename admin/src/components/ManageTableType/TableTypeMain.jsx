...
const TableTypeMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findTableType = async () => {
                try {
                    const searchRes = await TableTypeService.findTableTypeByIdOrName(search);
                    ...
                    setTableTypeList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - TableTypeMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getTableTypes = async () => {
            try {
                const TableTypeRes = await TableTypeService.getAllTableTypes();
    ...
    console.log("tableTypeList: ", tableTypeList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [tableTypeModal, setTableTypeModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setTableTypeModal(modal.tableType);
    }
    ...
    const tableTypePerPage = 12;
    const pageVisited = pageNumber * tableTypePerPage;

    const tableTypeListFiltered = tableTypeList
        .slice(pageVisited, pageVisited + tableTypePerPage)
        .map((tableType, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{pageNumber * tableTypePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{tableType.table_type_id}</Td>
                    <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{tableType.table_type_name}</Td>
                    <Td
                        onClick={() => openModal({ type: "detailTableType", tableType: tableType })}
                        style={{ backgroundColor: tableType.table_type_state === 0 ? "var(--color-info)" : tableType.table_type_state === 1 ? "var(--color-danger)" : null }}>
                        {tableType.table_type_state === 0 ? "Đang hoạt động" : tableType.table_type_state === 1 ? "Ngưng hoạt động" : null}
                    </Td>
                    <Td className="warning">
                        <ButtonFix
                            onClick={() => openModal({ type: "updateTableType", tableType: tableType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "disableTableType", tableType: tableType })}
                        >
                            <PersonOffOutlined />
                        </ButtonDelete>
                    </Td>
                    <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "ableTableType", tableType: tableType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deleteTableType", tableType: tableType })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(tableTypeList.length / tableTypePerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Loại bàn - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Loại bàn - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                tableTypeList.length > 0
                                    ?
                                    tableTypeListFiltered
                                    :
                                    (tableTypeList.map((tableType, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{tableType.table_type_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailTableType", tableType: tableType })}>{tableType.table_type_name}</Td>
                                                <Td
                                                    onClick={() => openModal({ type: "detailTableType", tableType: tableType })}
            ...
            <Modal
                ...
                tableType={tableTypeModal}  //Dữ liệu bên trong modal
