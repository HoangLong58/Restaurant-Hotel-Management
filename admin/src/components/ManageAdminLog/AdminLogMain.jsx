...
const AdminLogMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findAdminLog = async () => {
                try {
                    const searchRes = await AdminLogService.findAdminLogByIdOrName(search);
                    ...
                    setAdminLogList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - AdminLogMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getAdminLogs = async () => {
            try {
                const adminLogRes = await AdminLogService.getAdminLogs();
    ...
    console.log("adminLogList: ", adminLogList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [adminLogModal, setAdminLogModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setAdminLogModal(modal.adminLog);
    }
    ...
    const adminLogPerPage = 12;
    const pageVisited = pageNumber * adminLogPerPage;

    const adminLogListFiltered = adminLogList
        .slice(pageVisited, pageVisited + adminLogPerPage)
        .map((adminLog, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}>{pageNumber * adminLogPerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}>{adminLog.admin_log_date}</Td>
                    <Td
                        onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}
                        style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <ImgDanhMuc src={adminLog.employee_image} />
                    </Td>
                    <Td onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}>{adminLog.employee_first_name + " " + adminLog.employee_last_name + " " + adminLog.admin_log_content}</Td>
                    <Td
                        style={{ backgroundColor: adminLog.admin_log_type === "CREATE" ? "var(--color-success)" : adminLog.admin_log_type === "UPDATE" ? "var(--color-warning)" : adminLog.admin_log_type === "DELETE" ? "var(--color-danger)" : null }}
                        onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}
                    >{adminLog.admin_log_type}</Td>
                </Tr>
            );
        }
        );
    ...
    const pageCount = Math.ceil(adminLogList.length / adminLogPerPage);
    ...
    return (
        <Container>
            ...
            <RecentOrders>
                <H2>Nhật ký hoạt động hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                adminLogList.length > 0
                                    ?
                                    adminLogListFiltered
                                    :
                                    (adminLogList.map((adminLog, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}>{adminLog.admin_log_date}</Td>
                                                <Td
                                                    onClick={() => openModal({ type: "detailAdminLog", adminLog: adminLog })}
                                                    style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Modal
                ...
                adminLog={adminLogModal}  //Dữ liệu bên trong modal
