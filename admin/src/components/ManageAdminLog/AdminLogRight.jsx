...
const AdminLogRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng thiết bị
    const [quantityAdminLog, setQuantityAdminLog] = useState();
    useEffect(() => {
        const getQuantityAdminLog = async () => {
            try {
                const quantityAdminLogRes = await AdminLogService.getQuantityAdminLog();
                setQuantityAdminLog(quantityAdminLogRes.data.data.quantityAdminLog);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
    ...
    console.log("Số lượng loại thiết bị: ", quantityAdminLog);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG HOẠT ĐỘNG</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityAdminLog}</h3>
                    </ItemRight>
                </Item>
                {/* <Item className="add-product"
                    onClick={() => openModal({ type: "createAdminLog" })}
                >
                    <Add />
