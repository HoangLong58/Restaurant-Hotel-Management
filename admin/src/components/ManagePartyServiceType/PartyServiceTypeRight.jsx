...
const PartyServiceTypeRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng thiết bị
    const [quantityPartyServiceType, setQuantityPartyServiceType] = useState();
    useEffect(() => {
        const getQuantityPartyServiceType = async () => {
            try {
                const quantityPartyServiceTypeRes = await PartyServiceTypeService.getQuantityPartyServiceType();
                setQuantityPartyServiceType(quantityPartyServiceTypeRes.data.data.quantityPartyServiceType);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
        getQuantityPartyServiceType();
    }, [reRenderData]);
    console.log("Số lượng loại DỊCH VỤ TIỆC: ", quantityPartyServiceType);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG LOẠI DỊCH VỤ TIỆC</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityPartyServiceType}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createPartyServiceType" })}
                >
                    <Add />
                    <h3>Thêm Loại Dịch vụ tiệc</h3>
                </Item>
            </SalesAnalytics>
...
export default PartyServiceTypeRight;