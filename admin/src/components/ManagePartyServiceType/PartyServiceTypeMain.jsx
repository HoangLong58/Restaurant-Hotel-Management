...
const PartyServiceTypeMain = ({ reRenderData, setReRenderData }) => {
    ...
    const handleSeach = (e) => {
        if (isSearch === false) {
            ...
            // Thực hiện tìm kiếm
            console.log(search);
            const findPartyServiceType = async () => {
                try {
                    const searchRes = await PartyServiceTypeService.findPartyServiceTypeByIdOrName(search);
                    ...
                    setPartyServiceTypeList(searchRes.data.data);
                    // Toast
    ...
    const handleClose = () => {
        ...
        setReRenderData(prev => !prev); //Render lại csdl ở Compo cha là - PartyServiceTypeMain & DanhMucRight.jsx
    }
    ...
    useEffect(() => {
        const getPartyServiceTypes = async () => {
            try {
                ...
                console.log("Lỗi lấy party service type: ", err);
            }
        }
        handleLoading();
        getPartyServiceTypes();
    }, [reRenderData]);
    console.log("partyServiceTypeList: ", partyServiceTypeList);

    // Modal
    const [showModal, setShowModal] = useState(false);
    const [typeModal, setTypeModal] = useState("")
    const [partyServiceTypeModal, setPartyServiceTypeModal] = useState(null);

    const openModal = (modal) => {
        setShowModal(prev => !prev);
        setTypeModal(modal.type);
        setPartyServiceTypeModal(modal.partyServiceType);
    }
    ...
    const partyServiceTypePerPage = 12;
    const pageVisited = pageNumber * partyServiceTypePerPage;

    const partyServiceTypeListFiltered = partyServiceTypeList
        .slice(pageVisited, pageVisited + partyServiceTypePerPage)
        .map((partyServiceType, key) => {
            return (
                <Tr>
                    <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{pageNumber * partyServiceTypePerPage + (key + 1)}</Td>
                    <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{partyServiceType.party_service_type_id}</Td>
                    <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{partyServiceType.party_service_type_name}</Td>
                    <Td
                        onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}
                        style={{ backgroundColor: partyServiceType.party_service_type_state === 0 ? "var(--color-info)" : partyServiceType.party_service_type_state === 1 ? "var(--color-danger)" : null }}>
                        {partyServiceType.party_service_type_state === 0 ? "Đang hoạt động" : partyServiceType.party_service_type_state === 1 ? "Ngưng hoạt động" : null}
                    </Td>
                    <Td className="warning">
                        <ButtonFix
                            onClick={() => openModal({ type: "updatePartyServiceType", partyServiceType: partyServiceType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "disablePartyServiceType", partyServiceType: partyServiceType })}
                        >
                            <PersonOffOutlined />
                        </ButtonDelete>
                    </Td>
                    <Td className="primary">
                        <ButtonInfo
                            onClick={() => openModal({ type: "ablePartyServiceType", partyServiceType: partyServiceType })}
                        >
                    ...
                    <Td className="primary">
                        <ButtonDelete
                            onClick={() => openModal({ type: "deletePartyServiceType", partyServiceType: partyServiceType })}
                        >
                            <DeleteSweepOutlined />
                        </ButtonDelete>
                    </Td>
    ...
    const pageCount = Math.ceil(partyServiceTypeList.length / partyServiceTypePerPage);
    ...
    return (
        <Container>
            <H2>Quản lý Loại Dịch vụ tiệc - Nhà hàng</H2>
            ...
            <RecentOrders>
                <H2>Loại Dịch vụ tiệc - Nhà hàng hiện tại</H2>
                <Table>
                    <Thead>
                        <Tr>
                            <Th>STT</Th>
                            <Th>Mã loại Dịch vụ tiệc</Th>
                            <Th>Tên loại Dịch vụ tiệc</Th>
                            <Th>Trạng thái</Th>
                            <Th>Chỉnh sửa</Th>
                            <Th>Vô hiệu hóa</Th>
                            <Th>Mở khóa</Th>
                        ...
                        {noResultFound ? (
                            ...
                            : isLoading ? (
                                ...
                                partyServiceTypeList.length > 0
                                    ?
                                    partyServiceTypeListFiltered
                                    :
                                    (partyServiceTypeList.map((partyServiceType, key) => {
                                        return (
                                            <Tr>
                                                <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{(key + 1)}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{partyServiceType.party_service_type_id}</Td>
                                                <Td onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}>{partyServiceType.party_service_type_name}</Td>
                                                <Td
                                                    onClick={() => openModal({ type: "detailPartyServiceType", partyServiceType: partyServiceType })}
            ...
            <Modal
                ...
                partyServiceType={partyServiceTypeModal}  //Dữ liệu bên trong modal
...
export default PartyServiceTypeMain;