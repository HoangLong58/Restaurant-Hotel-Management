...
const PartyBookingTypeRight = ({ reRenderData, setReRenderData }) => {
    // Thứ ngày tháng
    let today = new Date();
    ...
    // Số lượng thiết bị
    const [quantityPartyBookingType, setQuantityPartyBookingType] = useState();
    useEffect(() => {
        const getQuantityPartyBookingType = async () => {
            try {
                const quantityPartyBookingTypeRes = await PartyBookingTypeService.getQuantityPartyBookingType();
                setQuantityPartyBookingType(quantityPartyBookingTypeRes.data.data.quantityPartyBookingType);
            } catch (err) {
                console.log("Lỗi: ", err);
            }
        }
        getQuantityPartyBookingType();
    }, [reRenderData]);
    console.log("Số lượng loại đặt tiệc: ", quantityPartyBookingType);
        ...
        <Container>
            ...
            <SalesAnalytics>
                ...
                <Item className="online">
                    ...
                    <ItemRight>
                        <Info>
                            <h3>SỐ LƯỢNG LOẠI ĐẶT TIỆC</h3>
                            <small class="text-muted">{ngaythangnam}</small>
                        </Info>
                        <h3 className="success" style={{ fontSize: "1.2rem" }}>{quantityPartyBookingType}</h3>
                    </ItemRight>
                </Item>
                <Item className="add-product"
                    onClick={() => openModal({ type: "createPartyBookingType" })}
                >
                    <Add />
                    <h3>Thêm Loại đặt tiệc</h3>
                </Item>
            </SalesAnalytics>
...
export default PartyBookingTypeRight;